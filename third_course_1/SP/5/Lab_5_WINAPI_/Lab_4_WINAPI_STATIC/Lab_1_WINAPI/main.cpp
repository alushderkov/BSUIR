...
DWORD WINAPI MyThreadFunction(LPVOID lpParam) { 
    ...
    FileData* fileData = static_cast<FileData*>(lpParam);

    // Assuming fileData contains a pointer to the vector
    std::vector<FileData>& unsortedData = *reinterpret_cast<std::vector<FileData>*>(fileData);


    MergeSort(unsortedData, 1, unsortedData.size() - 1, std::stoi(unsortedData[0].fileName));
    for (int i = 0; i < std::stoi(unsortedData[0].fileName); i++) {
        for (int j = 0; j < unsortedData.size(); j++) {
            if (j == 10) break;
            std::wcout << unsortedData[j].fileDate << std::endl;
            std::wcout << L"------------------" << std::endl;

        }
        std::wcout << unsortedData.size() << std::endl;

    }
    return 0;
}
/*
std::vector<FileData> SortInParalelThreads(std::vector<FileData> unsortedData, int columnIndex, const int K)
{
    int dataSize = static_cast<int>(unsortedData.size());
    int partSize = dataSize / K;

    std::vector<HANDLE> hThreadArray(K);
    std::vector<DWORD>   dwThreadIdArray(K);
    std::vector<std::vector<FileData>> filedatas(K + 1);

    for (int i = 0; i < K; i++) {
        filedatas[i].push_back({ std::to_wstring(columnIndex) });
    }
    ...
    for (int i = 0; i < K; i++) {
        ...
        for (int j = start; j < end; j++) {
            filedatas[i].push_back(unsortedData[j]);
        }
        std::wcout << filedatas[i].size() << std::endl;

        hThreadArray[i] = CreateThread(
            NULL,                                   // LPSECURITY_ATTRS default security attributes
            0,                                      // SIZE_T use default stack size  
            MyThreadFunction,                       // LPTHREAD_START_ROUTINE thread function name
            static_cast<void*>(&filedatas[i]),      // LPVOID argument to thread function 
            0,                                      // DWORD use default creation flags 
            &dwThreadIdArray[i]);                   // LPDWORD returns the thread identifier 

    }


    for (int i = 0; i < hThreadArray.size(); i++) {
        auto& pi = hThreadArray[i];

        WaitForSingleObject(pi, INFINITE);
        CloseHandle(pi);

        if (i == 1) {
            unsortedData = MergeTwoSortedLists(filedatas[i], filedatas[i - 1], columnIndex);
        }
        else if (i > 2) {
            unsortedData = MergeTwoSortedLists(filedatas[i], unsortedData, columnIndex);
        }
    }


    for (int i = 0; i < K; i++) {
        filedatas[i].erase(filedatas[i].begin());
    }
...
std::vector<FileData> SortInParallelThreads(std::vector<FileData> unsortedData, int columnIndex, const int K)
{
    int dataSize = static_cast<int>(unsortedData.size());
    int partSize = dataSize / K;
    ...
    for (int i = 0; i < K; i++) {
        ...
        hThreadArray[i] = CreateThread(
            ...
            static_cast<void*>(&filedatas[i]),      // LPVOID argument to thread function 
            0,                                      // DWORD use default creation flags 
            NULL);                                  // LPDWORD returns the thread identifier 

    }

    int mergedCount = 0;  
    while (mergedCount < K) {
        DWORD result = WaitForMultipleObjects(K, hThreadArray.data(), FALSE, INFINITE);
        int index = result - WAIT_OBJECT_0; 
            
        if (mergedCount == 0) {
             unsortedData = filedatas[index]; 
        }
        else {
             unsortedData = MergeTwoSortedLists(unsortedData, filedatas[index], columnIndex);
        }

        CloseHandle(hThreadArray[index]);
        hThreadArray.erase(hThreadArray.begin() + index);
        mergedCount++;  
    }
    ...
    */
    ...
    return MergeKSortedLists(filedatas, columnIndex);
}
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        hCustomListView->AddColumn(0, L"File name");
        hCustomListView->AddColumn(3, L"File name");
        hCustomListView->AddColumn(2, L"File size");
        hCustomListView->AddColumn(5, L"File size");
        hCustomListView->AddColumn(1, L"File date");
        hCustomListView->AddColumn(4, L"File date");

        SendMessage(hCustomListView->GetHWND(), WM_SETREDRAW, FALSE, 0);
    
        int size = static_cast<int>(unsortedData.size());

        hCustomListView->AddItem(0, 0, L"Files: "+ std::to_wstring(size));
        for (int i = 0; i < size; i++) {
            int row = i + 1;
            hCustomListView->AddItem(row, 0, unsortedData[i].fileName);
            hCustomListView->AddItem(row, 3, unsortedData[i].fileName);
            hCustomListView->AddItem(row, 1, unsortedData[i].fileDate);
            hCustomListView->AddItem(row, 4, unsortedData[i].fileDate);
            hCustomListView->AddItem(row, 2, unsortedData[i].fileSize);
            hCustomListView->AddItem(row, 5, unsortedData[i].fileSize);
        }
    ...
    case WM_HEADER_CLICK: 
        {
            ...
            auto sortedData = SortInParallelThreads(unsortedData, columnIndex+1, K);
            SendMessage(hCustomListView->GetHWND(), WM_SETREDRAW, FALSE, 0);

            auto size = static_cast<int>(sortedData.size());
            hCustomListView->AddItem(0, 3, L"Files: "+ std::to_wstring(size));
            for (auto i = 0; i < size; i++) {
                int row = i + 1;
                hCustomListView->AddItem(row, 3, sortedData[i].fileName);
                hCustomListView->AddItem(row, 4, sortedData[i].fileDate);
                hCustomListView->AddItem(row, 5, sortedData[i].fileSize);
            }
