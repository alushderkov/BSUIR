...
void Table::UpdateItemHeight() {
    ...
    if (this->itemHeight * this->header->GetColumnCount() < screenh) {
        this->itemHeight = screenh / (this->rowCount + 1);
        this->header->SetColumnHeight(itemHeight + screenh % (this->rowCount + 1));
    }
...
void Table::OnPaint() {
    ...
    HBRUSH headerBrush = CreateSolidBrush(RGB(191, 191, 191));
    ...
    for (int j = 0; j < this->header->GetColumnCount(); j++) {
        ...
        if (j == selectedColumn && selectedItem == -1) {
            HBRUSH selectionBrush = CreateSolidBrush(RGB(191, 191, 191));
    ...
    for (int i = scrollPos; i < items.size() && (i - scrollPos + 1) * itemHeight < clientRect.bottom; ++i) {
        ...
        for (int j = 0; j < this->header->GetColumnCount(); j++) {
            ...
            RECT highLightRect = { x + 1, itemRect.top, x + this->header->GetColumnWidths()[j], itemRect.bottom - 1 };

            if (i == selectedItem && j == selectedColumn) {
                HBRUSH selectionBrush = CreateSolidBrush(RGB(255, 255, 255));
                FillRect(memDC, &highLightRect, selectionBrush);
                DeleteObject(selectionBrush);
            }
        ...
        LineTo(memDC, clientWidth, itemRect.bottom - 1);  //   
