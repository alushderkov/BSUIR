...
#define CUSTOM_LIST_VIEW_ROW_COUNT 20
#define CUSTOM_LIST_VIEW_COL_COUNT 5
    ...
    case WM_KEYDOWN:
        switch (wParam)
        {
            case VK_RETURN: 
            {
                ...
                HWND hEdit = GetDlgItem(hFontChildWindow, ID_FONTS_LOAD_EDIT); 
                SetFocus(hEdit); 
...
LRESULT CALLBACK ChildWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) 
{
    ...
    case WM_PAINT:
    {
        ...
        FillRect(hdc, &ps.rcPaint, (HBRUSH)CreateSolidBrush(RGB(0, 188, 219)));
        EndPaint(hwnd, &ps);
    }
    return 0;
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        hHeaderFont = nullptr;
        //create_font_with_params_func(gFontPaths[0], GetFileNameWithoutExtension(gFontPaths[0]), DEFAULT_FONT_SIZE * 2);
        hCellFont = nullptr;
        //create_font_with_params_func(gFontPaths[3], GetFileNameWithoutExtension(gFontPaths[3]), DEFAULT_FONT_SIZE);

            
        register_custom_list_class_func((HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), WC_CUSTOM_LIST_VIEW);
        hCustomListView = create_custom_table_func(hwnd, CUSTOM_LIST_VIEW_ROW_COUNT, CUSTOM_LIST_VIEW_COL_COUNT, hHeaderFont, hCellFont);
        ...
        for (int i = 0; i < CUSTOM_LIST_VIEW_COL_COUNT; i++)
        {
            add_column_func(hCustomListView, i, (L"Header" + std::to_wstring(i)).c_str());
        }

        for (int i = 0; i < CUSTOM_LIST_VIEW_ROW_COUNT; i++)
        {
            for (int j = 0; j < CUSTOM_LIST_VIEW_COL_COUNT; j++)
            {
                add_item_func(hCustomListView, i, j, (L"File" + std::to_wstring(i)).c_str());
            }
        }
    ...
    case WM_HEADER_CLICK: 
    {
        ...
        std::wstring message = L"WM_HEADER_CLICK - Column: " + std::to_wstring(columnIndex) +
            L"\nTime: " + GetCurrentTimeNew();
        MessageBox(NULL, message.c_str(), L"Header Click", MB_OK | MB_TASKMODAL);
    }
    break;
...
{
    hInstLibrary = LoadLibrary(L"D:\\3Curse\\SP\\4\\Lab4\\TABLE_DLL.dll");
